...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST) && isset($_POST['html']) && isset($_POST['css']) && isset($_POST['name']) && !empty($_POST['html']) && !empty($_POST['css'])) {
    ...
    // Verifica se è stato fornito un nuovo nome del template, altrimenti mantieni il nome esistente
    if(empty($templateName)) {
        // Query per ottenere il nome del template esistente
        $sql_get_template_name = "SELECT name FROM templates WHERE id=?";
        $stmt = $conn->prepare($sql_get_template_name);
        $stmt->bind_param("i", $template_id);
        $stmt->execute();
        $result = $stmt->get_result();
        $existingTemplateName = $result->fetch_assoc()['name'];

        // Se il nome del template esistente è valido, utilizzalo
        if(!empty($existingTemplateName)) {
